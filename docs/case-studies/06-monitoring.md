...
| 방법 | 장점 | 단점 | 선택 |
|---|---|---|---|
| 서비스 포트에 열고 Nginx에서 차단 | 설정이 적음 | 리버스 프록시가 없는 배포에서는 **그대로 노출** | ❌ |
...
> **후속(2026-08-21)** — 이 문제는 [CS 11](11-rate-limiter-concurrency.md)에서 처리했습니다.
> 당시 40.2라는 값은 뷰포트 1회가 외부 API를 41번 부르던 시점의 것이라
> 지금 코드(10번)로는 재현되지 않습니다. 리미터를 직접 재는 도구로 다시 측정해
> **동시 16건 74.0 → 5.0 req/s**로 고쳤습니다.
