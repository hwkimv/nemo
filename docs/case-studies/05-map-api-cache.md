...
> **업데이트(2026-08-19)** — 아래 본문은 8월 14일 당시의 직접 구현 캐시 분석이다. 호출 수를 먼저 10회까지 줄인 뒤 Caffeine 교체를 완료했으며, 문서 마지막 후속 조치 3에 연결했다.
...
"Redis 분산 캐시"를 넣을까 고민했습니다. 그런데 넣기 전에 답해야 할 질문이 있었습니다.
...
**Caffeine도 Redis도 아닙니다.** 그렇게 부르지 않으려면 실체를 먼저 확인해야 했습니다.
...
## 후속 조치 3 — 직접 구현 캐시를 Caffeine으로 교체 (2026-08-19)

호출 수 자체를 41회에서 10회로 먼저 줄인 뒤, 남아 있던 메모리 관리 문제를 해결했다.

- 기존 완성 URI key, Local/Reverse 공통 범위, TTL 120초와 TTL 0 OFF 설정 유지
- `expireAfterWrite`로 만료 정책 명시
- `maximumSize=1000`으로 무제한 증가 방지
- `recordStats()`로 hit/miss/eviction 확인 가능
- 단일 인스턴스 구조이므로 Redis는 도입하지 않음

상세 구현·회귀 테스트·스텁 측정 결과는
[`2026-08-19-caffeine-local-cache.md`](../evidence/2026-08-19-caffeine-local-cache.md)에 기록했다.
---

## 후속 조치 4 — 하나의 캐시를 데이터 성격에 따라 둘로 나눴다 (2026-08-20)

후속 조치 3에서 Caffeine으로 갈아탔지만 **캐시는 여전히 하나**였다. 그 다음 문제가 남아 있었다.
...
그리고 `recordStats()`가 켜져 있어도 통계가 **하나로 합쳐져** 나오기 때문에,
**어느 쪽이 외부 호출 비용을 쓰는지 구분할 수 없었습니다.**

후속 조치 3이 `maximumSize`와 `recordStats`를 붙여 메모리 문제는 막았지만,
**두 데이터를 다른 정책으로 관리하는 것**과 **어느 쪽이 비용을 쓰는지 구분하는 것**은
캐시가 하나인 이상 불가능했습니다.
...
동일 요청 10회, **실제 NAVER API**, 조건마다 앱 재시작(캐시 비움):

| 조건 | local TTL | reverse TTL | 외부 Local | 외부 Reverse | 총 | 10회 소요 | 결과 |
|---|---:|---:|---:|---:|---:|---:|---:|
| Local 캐시 OFF | 0 | 1800 | **90** | 1 | 91 | 18,907ms | 3건 |
| 둘 다 ON | 300 | 1800 | **9** | 1 | **10** | 2,058ms | 3건 |
| Reverse 캐시 OFF | 300 | 0 | 9 | **10** | 19 | 3,705ms | 3건 |

**한쪽을 꺼도 다른 쪽이 그대로 동작합니다.** 이게 분리의 핵심입니다.
그리고 **세 조건 모두 결과가 3건으로 같습니다** — 캐시는 성능 장치이지 기능이 아닙니다.
...
- `local-search` **98.9%** — 지도를 옮겨도 지역명이 같으면 검색어가 같아 적중
- `reverse-geocoding` **69.9%** — 좌표가 곧 키라서 지도를 움직이면 새 키가 생김

**두 캐시가 다르게 동작한다는 것이 숫자로 보입니다.**
하나의 캐시였다면 이 둘은 합쳐져 하나의 값이 되고, 어느 쪽이 문제인지 알 수 없었습니다.

적중률만으로는 부족해서 **실제로 네이버로 나간 호출 수**도 같이 냈습니다.
비용은 적중률이 아니라 나간 호출에서 발생하고, 캐시를 끄면 적중률 지표 자체가 사라집니다.

![외부 호출](../evidence/screenshots/grafana-naver-api-calls.png)
