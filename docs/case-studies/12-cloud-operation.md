...
| 시나리오 | 결과 | exit | A. 감지 | B. 롤백→UP | **C. 실제 중단** | D. 전체 |
|---|---|---:|---:|---:|---:|---:|
| A. 정상 배포 | 성공 | **0** | — | — | **23.7s** | 24.7s |
| B. `/readyz` 없는 이미지 | 롤백 성공 | **1** | 29.1s | 24.0s | **52.4s** | 53.1s |
| C1. `docker run` 지속 실패 | 롤백 대상 없음 | **2** | 1.5s | — | 미복구 | 1.6s |
| C2. `docker run` 1회 실패 | **롤백 성공** | **1** | 1.4s | 23.8s | **24.4s** | 25.2s |
| D. 롤백 이미지도 기동 실패 | 사람 개입 | **2** | 1.6s | — | 미복구 | 1.8s |
| E. 동시 배포 2개 | 하나만 진행 | **3** | — | — | — | — |
...
| | 리뷰 전 | 지금 |
|---|---:|---:|
| 실패 감지 (A) | **123s** (120s 상한을 다 채움) | **29.1s** |
| 롤백 → readiness UP (B) | 26s | 24.0s |
| **실제 서비스 중단 (C)** | **약 148s** | **52.4s** |
